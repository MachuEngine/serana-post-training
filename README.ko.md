...
**post-training 스택**(prompt → CPT → SFT → DPO)을 **일반 목적 LLM의 페르소나 일관성 문제**에 적용한 end-to-end 프로젝트다. **24GB GPU 한 장**(NVIDIA L4)만으로 전체를 구축하고 측정했다.

테스트베드는 *엘더스크롤 5: 스카이림*(던가드)의 NPC **세라나**다:
- 방대한 기존 대사 데이터
- 뚜렷한 성격
- 그리고 자연스러운 지식 경계 — 수천 년을 잠들어 있던 ~4,000살 뱀파이어라, 현대 세계를 전혀 모른다고 가정할 수 있음
