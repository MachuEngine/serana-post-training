...
이 프로젝트는 일련의 학습 기법을 거치면서 모델이 캐릭터를 얼마나 더 잘 유지하게 되는지를 — 시연 영상이 아니라 실제 숫자로 — 학습하고 측정한다. 그리고 이 전체 과정을 **일반인도 구할 수 있는 GPU 한 장**(NVIDIA L4, 24GB — GPT 같은 모델을 학습시키는 하드웨어에 비하면 소박한 수준)만으로 해낸다.

**테스트 캐릭터는 세라나**, 비디오 게임 *엘더스크롤 5: 스카이림*(확장팩 *던가드*)에 나오는 NPC(플레이어가 조작하지 않는 게임 속 캐릭터)다. 아무 캐릭터나 고른 게 아니다:

- 그녀는 ~4,000살 된 뱀파이어로, 수 세기 동안 봉인된 채 잠들어 있었다 — 그래서 최근 사건이나 현대 세계에 대해 전혀 모르는 게 설정상 자연스럽다. 이게 깨끗하게 테스트할 수 있는 기준선을 만들어준다: 모델이 세라나가 몰라야 할 걸 아무렇지 않게 지어내지 않고, 실제로 "모른다"고 제대로 반응하는가?
- 게임 본편과 팬 위키에 걸쳐 이미 작성된 공식 대사가 방대하게 존재한다 — 처음부터 지어낸 게 아니라, 그녀의 말투를 학습할 수 있는 진짜 예시 텍스트가 있다는 뜻이다.
- 뚜렷하고 잘 문서화된 성격(건조하고 방어적이지만, 일단 믿게 된 상대에게는 은근히 다정함)을 가지고 있어서, 모델이 실제로 그 성격대로 답하는지 검증하기 쉽다.
