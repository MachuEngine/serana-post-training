...
`tracking.mode`가 `offline`이다. `.env`에 `WANDB_API_KEY`를 넣고
`wandb sync wandb/offline-run-*` 를 실행하면 두 구간이 하나의 곡선으로 올라가고,
`config/base.yaml`의 `mode: online`이 이후 실행부터 바로 적용된다.
재개 구조 자체는 두 모드에서 동일하므로, 이 실증의 결론은 바뀌지 않는다.
